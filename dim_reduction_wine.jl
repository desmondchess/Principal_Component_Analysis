...
wine_data = CSV.read("wine.data", DataFrame, header = header)
...
# ╔═╡ c4d9a890-d0a2-45cd-8303-3745ad9250bf
col_length = ncol(select(wine_data, Not("Variety")))

# ╔═╡ e9cc8594-f49b-493a-a817-0886c3417269
feature_matrixX= Matrix(wine_data[:, Not(["Variety"])])
...
# ╔═╡ 8000cbcc-8fa8-4b8a-9fa2-0ea7f3241f70
begin
	standardized_X = zeros(length(feature_matrixX[:,1]),col_length)
	mean_s = zeros(col_length)
	sd_s = zeros(col_length)
	for col in 1:col_length
		mean_s[col] = mean([x for x in feature_matrixX[:,col]])
		sd_s[col] = std([x for x in feature_matrixX[:,col]])
		standardized_X[:,col] = (feature_matrixX[:,col] .- mean_s[col])./ sd_s[col]
	end
end

# ╔═╡ 7a6a455c-07c4-4925-9f4d-4dcfc490cf53
standardized_X
...
# ╔═╡ be9e8c32-00ff-45fb-8ee6-4c47de969112
begin
	model = PCA(n_components=2)
	new_X = model.fit_transform(standardized_X)
end
...
# ╔═╡ 2d4348b5-5cbf-4adc-b46a-2e5781002f26
colors = Dict(1 => "red", 2 => "green", 3 => "blue")

# ╔═╡ ec6289ee-74b5-419a-91f4-3d93cc2c7f19
begin
	fig = Figure()
	ax = Axis(fig[1,1], xlabel = "PC 1", ylabel = "PC 2", title = "Principal Component Visualization")
	x = new_X[:,1]
	y = new_X[:,2]
	label = wine_data[:,1]
	for row in 1:length(new_X[:,1])
		if label[row] == 1
			scatter!(x[row], y[row], color=colors[1], label="Variety 1")
		elseif label[row] == 2
			scatter!(x[row], y[row], color=colors[2], label="Variety 2")
		else	
			scatter!(x[row], y[row],color=colors[3], label="Variety 3")
		end
	end
...
# ╔═╡ ee697037-831a-4a54-b787-16ee92678985
md"Yeah, it was!"
...
# ╔═╡ 4f574ac6-b462-4541-8100-257a9c238cf0
sum(model.explained_variance_ratio_)

# ╔═╡ 0da761f1-2bf8-4f09-8b9e-c752ad33d6ef
md"
The percentage of variance captured by the first two pca is approximately 55.4%
"
